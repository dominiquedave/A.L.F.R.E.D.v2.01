...
from shared.models import AgentInfo, CommandResult, Message, MessageType
...
class Coordinator:
    ...
    async def register_agent(self, agent_info: AgentInfo):
        ...
        if agent_info.id in self.agents:
            logger.warning(
                f"Agent {agent_info.id} already exists - replacing with new registration"
            )

        self.agents[agent_info.id] = agent_info
        logger.info(
            f"Registered agent: {agent_info.name} ({agent_info.id}) at {agent_info.host}:{agent_info.port}"
        )
    ...
    async def discover_agents_broadcast(self) -> List[str]:
        ...
        try:
            ...
            broadcast_port = self.discovery_config.get("discovery_settings", {}).get(
                "broadcast_port", 5099
            )
            ...
            response_count = 0
            logger.info("👂 Listening for agent responses for 3 seconds...")
            ...
            while time.time() - start_time < 3:
                try:
                    ...
                    if response.get("type") == "agent_response":
                        ...
                        logger.info(
                            f"✅ Valid agent response: {agent_name} ({agent_os}) at {agent_host}"
                        )
                    else:
            ...
            logger.info(
                f"🏁 Broadcast discovery finished after {elapsed:.1f}s, received {response_count} responses"
            )
            sock.close()
        ...
        logger.info(
            f"📊 Broadcast discovery result: {len(discovered_hosts)} agents found: {discovered_hosts}"
        )
        return discovered_hosts
    ...
    async def discover_agents(self, host_range: List[str] = None):
        ...
        if not host_range:
            ...
            logger.info(
                f"🌍 Environment variables: AGENT_DISCOVERY_HOSTS='{env_hosts}', AGENT_NETWORK_CONFIG='{network_name}'"
            )
            ...
            if env_hosts:
                ...
                use_broadcast = (
                    str(
                        use_broadcast_env if use_broadcast_env is not None else use_broadcast_config
                    ).lower()
                    == "true"
                )

                logger.info(
                    f"📻 Broadcast setting - ENV: '{use_broadcast_env}', CONFIG: {use_broadcast_config}, FINAL: {use_broadcast}"
                )
                ...
                if use_broadcast:
                    ...
                    if broadcast_hosts:
                        ...
                        logger.info(
                            f"✅ Found {len(broadcast_hosts)} agents via broadcast: {broadcast_hosts}"
                        )
                ...
                if not host_range:
                    ...
                    scan_network = (
                        str(
                            scan_network_env
                            if scan_network_env is not None
                            else scan_network_config
                        ).lower()
                        == "true"
                    )

                    logger.info(
                        f"🌐 Network scanning setting - ENV: '{scan_network_env}', CONFIG: {scan_network_config}, FINAL: {scan_network}"
                    )
        ...
        async def check_host(host_port):
            ...
            async with semaphore:  # Limit concurrent connections
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        ...
                        async with session.get(url, timeout=3) as resp:
                            ...
                            if resp.status == 200:
                                ...
                                logger.info(
                                    f"✅ Successfully found and registered agent at {host_port}"
                                )
                                return True
                            else:
                                response_text = await resp.text()
                                logger.warning(
                                    f"⚠️ Agent at {host_port} returned status {resp.status}: {response_text}"
                                )
                    ...
                    logger.warning(
                        f"❌ Unexpected error connecting to {host_port}: {type(e).__name__}: {e}"
                    )
        ...
        logger.info(
            f"📊 Results: {successful_connections} agents found, {failed_connections} failed connections, {exceptions} exceptions"
        )
        logger.info(f"🤖 Registered agents: {len(self.agents)} total")
        for agent_id, agent_info in self.agents.items():
            logger.info(
                f"   - {agent_info.name} ({agent_id}) at {agent_info.host}:{agent_info.port} - healthy: {agent_info.is_healthy}"
            )
    ...
    async def health_check_agents(self, force: bool = False):
        ...
        async with self._health_check_lock:  # Ensure only one health check runs at a time
            ...
            if not force and time_since_last_check < timedelta(seconds=10):
                logger.debug(
                    f"Health check skipped - last check was {time_since_last_check.total_seconds():.1f}s ago"
                )
                return
            ...
            for agent_id, agent_info in list(self.agents.items()):
                ...
                for attempt in range(3):
                    try:
                        async with aiohttp.ClientSession() as session:
                            ...
                            logger.debug(
                                f"Health check attempt {attempt + 1}/3 for agent {agent_id} at {url}"
                            )
                            ...
                            async with session.get(url, timeout=5) as resp:
                                if resp.status == 200:
                                    ...
                                    logger.debug(
                                        f"Agent {agent_id} health check passed: {health_data.get('status', 'unknown')}"
                                    )
                                    ...
                                    logger.warning(
                                        f"Agent {agent_id} health check returned status {resp.status}"
                                    )

                    except Exception as e:
                        logger.warning(
                            f"Agent {agent_id} health check attempt {attempt + 1} failed: {e}"
                        )
                ...
                if agent_id in self.agents:
                    if not health_check_passed:
                        ...
                        logger.warning(
                            f"Agent {agent_id} marked as unhealthy after 3 failed attempts"
                        )
                    ...
                    logger.info(
                        f"Agent {agent_id} status: {status}, last seen: {time_since_last_seen.total_seconds():.1f}s ago"
                    )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(url, timeout=5) as resp:
                    ...
                    results["health_response"] = (
                        await resp.json() if resp.status == 200 else await resp.text()
                    )
        except Exception as e:
    ...
    async def parse_command(self, user_input: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"JSON parsing failed. Raw response: {response.choices[0].message.content}"
            )
    ...
    def select_agent(self, parsed_command: Dict) -> Optional[AgentInfo]:
        ...
        if target_os != "any":
            os_agents = [
                agent for agent in healthy_agents if agent.os_type.lower() == target_os.lower()
            ]
            if os_agents:
